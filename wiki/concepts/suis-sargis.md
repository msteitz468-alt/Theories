---
title: "Su'is and Sar'gis (clairvoyance and materialization)"
aliases:
  - "Su'is and Sar'gis (clairvoyance and materialization)"
  - "su'is"
  - "sar'gis"
  - "clairvoyance / materialization"
type: concept
also_known_as: ["su'is", "sar'gis", "clairvoyance / materialization"]
source_attribution: "[[Oahspe: A New Bible (Newbrough, 1882)]]"
last_updated: 2026-06-08
tags: ['concept', 'topic/oahspe']
---

## Definition from Source

**Su'is** is "clairaudience and clairvoyance — a person who can see with the eyes
closed, or one who can hear angel voices." **Sar'gis** is "both a materialised
angel, or a person in whose presence the angels can take on the semblance of
mortal forms" (a materialization medium). These are the faculties by which
prophets, seers, and mediums perceive the unseen world.

## Key Properties (from source)

- Su'is can be inborn (the prophets and lawgivers were "natural born su'is and
  sar'gis") or attained "by diet and by fasting."
- A su'is "can see vortexya," e.g. the light escaping a magnet in the dark.
- In the kosmon era these faculties spread widely as the heavens open (see
  [[In the Kosmon Era the Dead Commune With Mortals Face to Face]]), but the Book of Judgment warns that low or unclean sar'gis
  attract lying spirits (see [[Consulting Angels for Earthly Gain Delivers One to Lying Spirits]]).
- The degrees of [[Faithist Rites and the Degrees of Initiation]] teach the development of su'is and sar'gis.

## Related Concepts

- [[The Doctrine of Inspiration (Oahspe)]], [[Faithist Rites and the Degrees of Initiation]], [[Drujas, Fetals, and the Bondage of Hells]]

## Related Entities

- [[Oahspe: A New Bible (Newbrough, 1882)]]
- [[The Doctrine of Inspiration (Oahspe)]]
- [[Faithist Rites and the Degrees of Initiation]]
- [[Drujas, Fetals, and the Bondage of Hells]]
- [[In the Kosmon Era the Dead Commune With Mortals Face to Face]]
- [[Consulting Angels for Earthly Gain Delivers One to Lying Spirits]]

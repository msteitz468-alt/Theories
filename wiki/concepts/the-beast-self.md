---
title: "The Beast (Self) and Its Four Heads"
aliases:
  - "The Beast (Self) and Its Four Heads"
  - "Self"
  - "Satan"
  - "the four heads of the Beast"
type: concept
also_known_as: ["Self", "Satan", "the four heads of the Beast"]
source_attribution: "[[Oahspe: A New Bible (Newbrough, 1882)]]"
last_updated: 2026-06-08
tags: ['concept', 'topic/oahspe']
---

## Definition from Source

The **Beast** is self / selfishness — "the animal man. The earthly part of man."
In Oahspe's opening, "the Beast (self) rose up before man" and said, "Possess
thou whatsoever thou wilt," bringing war into the world. "And the Beast divided
itself into four great heads... BRAHMIN, BUDDHIST, CHRISTIAN and MOHAMMEDAN" —
the four idolatrous religions "whose trade was killing man." Satan is its captain:
"the captain of the selfish passions. Selfishness per se. Self."

## Key Properties (from source)

- Allegiance to the Beast makes one a [[The Uzians]] ("the numbers of the Beast");
  separation from it makes one a Faithist (see [[Mankind Divides Into Faithists and Uzians]]).
- "The four heads of the Beast shall be put away; and war shall be no more" in the
  kosmon era.
- Its "tables" — the times of war and darkness — run on 666 years, "SATAN'S
  TABLES, or the TIMES OF THE BEAST" (see [[The Tables of Prophecy (the prophetic numbers)]]).

## Related Concepts

- [[False Gods and Self-Godhood]], [[The Uzians]], [[Faithist Ethics: Vegetarianism and Non-Resistance]], [[The Tables of Prophecy (the prophetic numbers)]]

## Related Entities

- [[Oahspe: A New Bible (Newbrough, 1882)]]
- [[False Gods and Self-Godhood]]
- [[The Uzians]]
- [[Faithist Ethics: Vegetarianism and Non-Resistance]]
- [[The Tables of Prophecy (the prophetic numbers)]]
- [[Mankind Divides Into Faithists and Uzians]]
